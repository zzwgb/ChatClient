 ...
 * * 解决对Fragment的调度与重用问题，
 * * 达到最优的Fragment切换
 * Author: Create by Carson on 2020/11/2
 */
public class NavHelper<T> {
    ...
    private void doTabChanged(Tab<T> newTab, Tab<T> oldTab) {
        ...
        if (oldTab != null) {
            if (oldTab.fragment != null) {
                // 从界面移除，但是还在FragmentManager的缓存空间中
        ...
        if (newTab != null) {
            if (newTab.fragment == null) {
                ...
                //从FragmentManager的缓存空间中重新加载到界面上
    ...
    private void notifyTabSelect(Tab<T> newTab, Tab<T> oldTab) {
        Log.e("size----", fragmentManager.getFragments().size() + "");
        if (listener != null) {
